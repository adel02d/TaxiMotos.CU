...
bot.on("message:location", async (ctx) => {
  ...
  if (!pending) {
    await ctx.reply("⚠️ No tienes una solicitud activa\\. Usa /menu para empezar\\.");
    return;
  }
...
bot.callbackQuery("action:confirm_ride", async (ctx) => {
  ...
  if (DRIVER_GROUP_ID) {
    const driverKeyboard = new InlineKeyboard()
      .text("🛵 Aceptar Viaje", `driver:accept:${ride.id}`);
    try {
      await bot.api.sendMessage(
        ...
        `🏠 Recogida: ${ride.pickupAddress}\n` +
        `🎯 Destino: ${ride.dropoffAddress || "No especificado"}`,
        { parse_mode: "MarkdownV2", reply_markup: driverKeyboard }
      );
    } catch (error) {
...
bot.callbackQuery(/^driver:accept:(\d+)$/, async (ctx) => {
  ...
  const rideKeyboard = new InlineKeyboard()
  ...
  await ctx.reply(
    ...
    { parse_mode: "MarkdownV2", reply_markup: rideKeyboard }
  );
});
...
bot.callbackQuery(/^driver:start:(\d+)$/, async (ctx) => {
  ...
  if (ride) {
    await bot.api.sendMessage(
      ride.clientId,
      "🏁 *¡El viaje ha comenzado\\!*\\n\nEl conductor está en camino a tu destino\\.\\n\\n¡Buen viaje\\! 🛵",
      { parse_mode: "MarkdownV2" }
    );
  }
  const finishKeyboard = new InlineKeyboard()
    .text("✅ Finalizar Viaje", `driver:finish:${rideId}`);
  await ctx.reply(
    `🏁 *Viaje #${rideId} en curso*\\n\nPresiona ✅ cuando llegues al destino\\.`,
    { parse_mode: "MarkdownV2", reply_markup: finishKeyboard }
  );
});
...
bot.callbackQuery(/^driver:finish:(\d+)$/, async (ctx) => {
  ...
  await prisma.ride.update({
    where: { id: rideId },
    data: { status: "completed", fare: fare, completedAt: new Date() },
  });
  await prisma.payment.create({
    data: { rideId: rideId, amount: fare, method: "cash", status: "completed", paidAt: new Date() },
  });
  await prisma.driver.update({
    where: { telegramId: driverTelegramId },
  ...
  await bot.api.sendMessage(
    ...
    { parse_mode: "MarkdownV2", reply_markup: rateKeyboard }
  );
  await ctx.reply(
...
bot.callbackQuery(/^driver:cancel_ride:(\d+)$/, async (ctx) => {
  ...
  await prisma.ride.update({
    where: { id: rideId },
    data: { status: "pending", driverId: null, driverName: null },
  });
  await prisma.driver.update({
    where: { telegramId: driverTelegramId },
  ...
  if (ride && DRIVER_GROUP_ID) {
    ...
    try {
      ...
      console.error("Error re-publicando viaje:", error);
    }
  }
...
bot.callbackQuery(/^rate:(\d):(\d+)$/, async (ctx) => {
  ...
  if (ride?.driverId) {
    ...
    if (driver) {
      ...
      await prisma.driver.update({
        where: { telegramId: ride.driverId },
        data: { rating: newTotal / newCount },
      });
    }
  }
  await ctx.reply(
...
bot.callbackQuery("action:my_data", async (ctx) => {
  ...
  if (!user) {
    await ctx.reply("⚠️ No tienes datos registrados\\.");
    return;
  }
  await ctx.reply(
    "👤 *Mis Datos:*\\n\\n" +
    ...
    "Para actualizar tu teléfono contacta a soporte\\.",
    { parse_mode: "MarkdownV2" }
  );
});
...
bot.command("add_driver", async (ctx) => {
  ...
  if (args.length < 1) {
    await ctx.reply(
      "⚠️ Uso: /add\\_driver \\<TelegramID\\> \\<nombre\\> \\<teléfono\\> \\<matrícula\\>\n\n" +
  ...
  const name = args[1] || "Conductor";
  const phone = args[2] || null;
  const plate = args[3] || null;
  try {
    const driver = await prisma.driver.upsert({
      where: { telegramId },
      update: { firstName: name, phone: phone || undefined, motorcyclePlate: plate || undefined, isActive: true },
      create: { telegramId, firstName: name, phone: phone || undefined, motorcyclePlate: plate || undefined },
    });
    await ctx.reply(
...
bot.command("remove_driver", async (ctx) => {
  ...
  const args = ctx.message!.text!.split(" ").slice(1);
  if (args.length < 1) {
    await ctx.reply("⚠️ Uso: /remove\\_driver \\<TelegramID\\>", { parse_mode: "MarkdownV2" });
    return;
  }
  try {
...
bot.command("drivers", async (ctx) => {
  ...
  if (drivers.length === 0) {
    await ctx.reply("📭 No hay conductores registrados\\.");
    return;
  }
  ...
  const list = drivers
    ...
    .join("\n\n");
  await ctx.reply(`🏍️ *Conductores Activos \\(${drivers.length}\\):*\\n\n${list}`, {
    parse_mode: "MarkdownV2",
  });
});
...
bot.command("stats", async (ctx) => {
  ...
  await ctx.reply(
    "📊 *Estadísticas de TaxiMotos\\.CU*\\n\\n" +
    `🚗 Total viajes: ${totalRides}\n📅 Hoy: ${todayRides}\n✅ Completados: ${completedRides}\n` +
    `⏳ Pendientes: ${pendingCount}\n💰 Ingresos totales: ${totalEarnings._sum.fare || 0} CUP\n` +
    `💵 Hoy: ${todayEarnings._sum.fare || 0} CUP\n🏍️ Conductores: ${activeDrivers}`,
    { parse_mode: "MarkdownV2" }
  );
});
...
bot.command("broadcast", async (ctx) => {
  ...
  if (!text) {
    await ctx.reply("⚠️ Uso: /broadcast \\<mensaje\\>");
    return;
  }
  ...
  for (const user of users) {
    try {
      await bot.api.sendMessage(user.telegramId, `📢 *Anuncio:*\n\n${text}`, {
        parse_mode: "MarkdownV2",
      });
      sent++;
    } catch { /* usuario bloqueó al bot */ }
  }
  await ctx.reply(`📢 Mensaje enviado a ${sent}/${users.length} usuarios\\.`);
});
...
async function main() {
  console.log("🛵 TaxiMotos.CU Bot iniciando...");
  try {
  ...
  await bot.start({
    onStart: (info) => {
      console.log(`✅ Bot iniciado como @${info.username}`);
    },
  });
}
